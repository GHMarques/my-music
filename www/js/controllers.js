...
.controller('PlaylistCtrl', function($scope, PlaylistService, $stateParams) {
  $scope.playlist = [];
  PlaylistService.getPlaylist($stateParams.playlistId).then(playlist => $scope.playlist = playlist);
})

.controller('BrowseMusicCtrl', ['$scope', '$http', function ($scope, $http) {
  'use strict';
  $http.get('https://mah-music-api.herokuapp.com/songs').success(function(data){
    $scope.musics = data;
  });
}]);
